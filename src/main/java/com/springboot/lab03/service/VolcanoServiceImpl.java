...
public class VolcanoServiceImpl implements VolcanoService {
    ...
    private final String volcanoListEndpoint = "/hazard-service/api/v1/volcanoes"; // 목록 API 엔드포인트
    private final String volcanoDetailEndpoint = "/hazard-service/api/v1/volcanoes/{id}/info"; // 상세 API 엔드포인트
    ...
    /**
     * 화산 데이터 목록 조회 (페이징 및 검색 조건 적용).
     * API 호출 후 클라이언트 측 추가 필터링(날짜, 고도, 위/경도) 수행.
     */
    @Override
    public PageDTO<Volcano> getData(SearchCriteria criteria) {
        ...
        if (apiResponse == null || apiResponse.getItems() == null) {
            System.err.println("API 응답 없음 - Volcano");
            return new PageDTO<>(); // 빈 결과 반환
        }

        // 클라이언트 측 필터링
        List<Volcano> items = apiResponse.getItems();
        List<Volcano> filteredItems = filterByDate(items, criteria.getMinDate(), criteria.getMaxDate());
        filteredItems = filterByElevation(filteredItems, criteria.getMinElevation());
    ...
    public Volcano getDataById(int id) {
        ...
        try {
            return restTemplate.getForObject(url, Volcano.class);
        } catch (HttpClientErrorException.NotFound nf) {
             System.err.println("Volcano not found: " + id);
             return null;
        } catch (RestClientException e) {
            System.err.println("Error fetching volcano detail (ID: " + id + "): " + e.getMessage());
            return null;
        }
    }
    ...
    private LocalDate getItemDate(Volcano item) {
        ...
        try {
            ...
            // System.err.println("Error creating date for Volcano ID: " + item.getId());
            return null;
        }
    }

    /** 날짜 범위로 목록 필터링 */
    private List<Volcano> filterByDate(List<Volcano> items, String minDateStr, String maxDateStr) {
    ...
    /** 최소 고도로 목록 필터링 */
    private List<Volcano> filterByElevation(List<Volcano> items, Integer minElevationCriteria) {
        if (minElevationCriteria == null) return items;
        return items.stream()
    ...
    /** 위/경도 범위로 목록 필터링 */
    private List<Volcano> filterByLocation(List<Volcano> items, SearchCriteria criteria) {
